...
public class FormationControllerWithWaves : MonoBehaviour {
    ...
    public int waves;                   //  Number of waves until level is finished
	...
	void Update () {
		...
		if (AllMembersDead () && waves > 0) {
			SpawnUntilFull ();	//	Previously SpawnEnemies ();
            waves--; // We decrease our waves counter
		}
        //	If all of the enemy ships are dead and there are no more waves
        //  We load the Win screen
        if (AllMembersDead() && waves == 0)
        {
            LevelManager levelManager = GameObject.Find("LevelManager").GetComponent<LevelManager>();
            levelManager.LoadLevel("Win Screen"); //real win screen is missing in current project
        }
    }
